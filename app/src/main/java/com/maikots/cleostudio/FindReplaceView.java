...
public class FindReplaceView extends LinearLayout {
    ...
    public FindReplaceView(Context context, CodeEditorView editorAlvo) {
        ...
        // 🛠️ FIX 1: LayoutParams explícitos para garantir exibição na largura total
        ...
        setVisibility(View.GONE); // Inicia oculto
    ...
    private void destacarOcorrenciaAtual() {
        ...
        if (editable != null && pos + tamanhoTermo <= editable.length()) {
            // Move o cursor do editor para a posição
            editorAlvo.setSelection(pos);

            // Destaca visualmente o termo encontrado em laranja
            editable.setSpan(
    ...
    private void substituirAtual() {
        if (indiceAtual < 0 || posicoesEncontradas.isEmpty() || editorAlvo.getText() == null) return;
    ...
    private void substituirTudo() {
        ...
        if (termoOriginal.isEmpty() || editorAlvo.getText() == null) return;
